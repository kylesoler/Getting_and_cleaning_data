...
## Organize and combine into single data set
...
xDataSet_mean_and_std <- xDataSet[, grep("-(mean|std)\\(\\)", read.table("features.txt")[, 2])]
names(xDataSet_mean_std) <- read.table("features.txt")[grep("-(mean|std)\\(\\)", read.table("features.txt")[, 2]), 2] 
View(xDataSet_mean_and_std)
dim(xDataSet_mean_and_std)
## Descriptive activities
yDataSet[, 1] <- read.table("activity_labels.txt")[yDataSet[, 1], 2]
names(yDataSet) <- "Activity"
View(yDataSet)
## Label data with descriptive activity
names(subjectDataSet)<-"Subject"
Summary(subjectDataSet)
## organize into one data set
singleDataSet <- cbind(xDataSet_mean_and_std, yDataSet, subjectDataSet)
...
names(singleDataSet) <- gsub('Freq$',"Frequency",names(singleDataSet))
View(singleDataSet)
## create second tidy data set with averages
names(singleDataSet)
